...
The onboard display wiring is fixed by the Waveshare PCB and is encoded in [`dash/firmware/include/WaveshareDashTFTSetup.h`](../dash/firmware/include/WaveshareDashTFTSetup.h):
...
Dash also joins the station network configured by `APEXI_WIFI_STATION_SSID` and `APEXI_WIFI_STATION_PASSWORD` in the ignored `shared/libraries/AppSecrets.h`, using the same credentials as Logger. It requests DHCP with hostname `apexi-dash`, starts connecting without blocking the BLE/UI loop, and retries every 30 seconds while disconnected. The retry policy lives in `dash/firmware/include/DashWifiPolicy.h`. Empty station credentials leave only the recovery AP active. Never commit credentials or distribute locally built credential-bearing images publicly.
...
- Update the engineering conversion assumptions in [`shared/libraries/AppConfig.h`](../shared/libraries/AppConfig.h) if the module output scaling no longer matches the original shunt-based design.
...
3. Confirm the legacy directly wired TFT controller is ST7796S. If it is ILI9488 or another controller, update [`logger/firmware/include/LoggerDisplayTFTSetup.h`](../logger/firmware/include/LoggerDisplayTFTSetup.h).
