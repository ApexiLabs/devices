...
The classic ESP32 and its production-candidate compile use link-time size optimization and disable unused C++ exception/unwind metadata and Arduino library debug logging (`CORE_DEBUG_LEVEL=0`). Application system events and serial/status diagnostics remain available. `scripts/esp32-lto.py` aligns linker and archive tooling with these flags. The existing 2 MiB OTA slots and queue offsets are unchanged; builds fail if the application exceeds its slot. Enabling additional library debug output requires rechecking the size limit.
