...
value.className=v.valid&&!v.warning?'ok':'warn';row.append(name,value);box.append(row)}
...
const r=await fetch('/api/settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({csrf,refreshMs:Number(el('refresh-ms').value),slots:[el('slot0').value,el('slot1').value]})});
